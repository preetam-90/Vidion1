...
// Mock Data for News Videos (fallback)
const MOCK_NEWS_VIDEOS: Video[] = [
  {
    id: 'mock-news-1',
    title: 'Breaking: Major Economic Reform Announced by Finance Minister',
    ...
    description: 'Finance Minister announces sweeping economic reforms to boost growth and investment.',
    platform: 'youtube',
    category: 'news',
    likes: '42K',
    comments: '3.5K',
  ...
  {
    id: 'mock-news-2',
    title: 'Special Report: The Impact of Climate Change on Indian Agriculture',
    ...
    description: 'An in-depth analysis of how climate change is affecting farmers across India.',
    platform: 'youtube',
    category: 'news',
    likes: '36K',
    comments: '2.8K',
  ...
  {
    id: 'mock-news-3',
    title: 'Election Results Live: Counting Underway in Five States',
    thumbnail: 'https://i.ytimg.com/vi/tech-placeholder/maxresdefault.jpg', // Placeholder
    uploader: 'Aaj Tak',
    views: '3.2M',
    uploadDate: new Date(Date.now() - 3 * 86400000).toISOString(), // 3 days ago
    description: 'Live coverage of the assembly election results from five states with expert analysis.',
    platform: 'youtube',
    category: 'news',
    likes: '65K',
    comments: '8K',
...
export default function NewsPage() {
  ...
  const loadMoreVideos = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/youtube/trending?region=IN&videoCategoryId=25${nextPageToken ? `&pageToken=${nextPageToken}` : ''}`)
      
      if (!response.ok) {
        let errorMsg = 'Failed to fetch trending news videos';
        try {
        ...
        if (videos.length === 0) {
          console.warn("Using mock news videos as fallback");
          setVideos(MOCK_NEWS_VIDEOS);
          setLoading(false);
          setLoadingMore(false);
          return;
        }
      ...
      if (videos.length === 0) {
        console.warn("API fetch failed, using mock news videos as fallback", err);
        setVideos(MOCK_NEWS_VIDEOS);
      } else {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Trending News Videos in India</h1>
      {videos.length > 0 ? (
        ...
        <div className="text-center py-12 text-muted-foreground">
          No trending news videos available
        </div>
      )}
