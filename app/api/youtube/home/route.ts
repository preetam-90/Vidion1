    ...
    const videos = searchData.items
      .map((item: any) => {
        ...
        return {
          ...
          thumbnail: detailSnippet?.thumbnails?.high?.url || searchSnippet?.thumbnails?.high?.url || '',
          ...
          viewCount: detailStats?.viewCount ? parseInt(detailStats.viewCount).toLocaleString() : 'N/A',
          duration: formatDuration(detailContent?.duration || ''),
        };
      })
